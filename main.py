...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        while True:
            ...
            original_speech = msg.get("voicePrompt", "").strip()
            user_speech = original_speech.lower()
            ...
            if booking_state["step"] is None:
                if "book" in user_speech or "appointment" in user_speech:
                    ...
                    if any(word in user_speech for word in [
                        ...
                        booking_state["service"] = original_speech
                        booking_state["step"] = "confirm_service"
                        reply = f"I heard {booking_state['service']}. Is that correct?"

                elif "close" in user_speech or "hours" in user_speech or "open" in user_speech:
                    ...
                    if "manicure" in user_speech:
                        reply = "Our manicure price is 25 dollars. Would you like to book an appointment?"
                    elif "pedicure" in user_speech:
                        reply = "Our pedicure price is 35 dollars. Would you like to book an appointment?"
                    elif "eyebrow" in user_speech:
                        reply = "Our eyebrow service price is 15 dollars. Would you like to book an appointment?"
                    elif "lip" in user_speech:
                        reply = "Our lip service price is 10 dollars. Would you like to book an appointment?"
                    elif "chin" in user_speech:
                        reply = "Our chin service price is 12 dollars. Would you like to book an appointment?"
                    else:
                        reply = "Prices vary by service. What service are you interested in? Manicure, pedicure, eyebrow, lip, or chin?"
                ...
                booking_state["day"] = original_speech
                ...
                booking_state["time"] = original_speech
                ...
                booking_state["name"] = original_speech
                ...
                if any(word in user_speech for word in ["yes", "correct", "right", "yeah", "yep"]):
                    ...
                    try:
                        create_booking_event(
                            ...
                            date=booking_state["day"],
                            time=booking_state["time"]
                        )
                        booking_state["booking_saved"] = True
                        calendar_message = "Your appointment has been saved to our calendar."
